...
def posicaoRealFinal(posicaoFinal):
...
def direcaoCorreta(minhaDirecao, movement, axis, alinhar): #define se o robô está virado para a direção correta, se não, corrige. Retorna direção atual
    if(axis == axisX):
        if(movement > 0): #Quer ir pra baixo (SUL) ###
            if(minhaDirecao == SUL):
                ...
                if(minhaDirecao == NORTE):
                    alinharOuNao(180, alinhar)
                if(minhaDirecao == OESTE):
                    alinharOuNao(90, alinhar)
...
def alinharOuNao(angulo, alinhar): 
    if(alinhar):
        TurnInSquare(angulo)
    else:
        if(angulo > 0):
            direcaoEGiro(esquerda, abs(angulo))
        if(angulo < 0):
            direcaoEGiro(direita, abs(angulo))

def turnTo(minhaDirecao, direcaoFinal, alinhar):
...
def notStockLocal(posicaoAtual, movement, axis):  #define se o robô está em volta de uma aŕea de carga onde não pode entrar
    if(axis == axisX):
        if(movement > 0): #Quer ir pra baixo (SUL)
            locaisDeCarga = [22, 23, 25, 26] #Lista de lugares que não podem ir pra baixo por conta do local de carga
        if(movement < 0): #Quer ir pra cima (NORTE)
...
def desvioAreaDeCarga(posicaoAtual, minhaDirecao): #desvia da área de carga e retorna nova posição e direção
    #Parte de cima
    if(posicaoAtual == 22):
        minhaDirecao = direcaoCorreta(minhaDirecao, -1, axisY, True)
        moverParaFrentePorQuadrado()
        minhaDirecao = direcaoCorreta(minhaDirecao, +1, axisX, True)
        moverParaFrentePorQuadrado()
        posicaoAtual = 31
    if(posicaoAtual == 23):
        minhaDirecao = direcaoCorreta(minhaDirecao, +1, axisY, True)
    ...
    #Parte de baixo
    if(posicaoAtual == 52):
        minhaDirecao = turnTo(minhaDirecao, OESTE, True)
        moverParaFrentePorQuadrado()
        minhaDirecao = turnTo(minhaDirecao, NORTE, True)
        moverParaFrentePorQuadrado()
        posicaoAtual = 41
    if(posicaoAtual == 53):
        minhaDirecao = turnTo(minhaDirecao, LESTE, True)
        moverParaFrentePorQuadrado()
        minhaDirecao = turnTo(minhaDirecao, NORTE, True)
        moverParaFrentePorQuadrado()
        posicaoAtual = 44
...
def corrigindoDirecao(moverX, moverY, posicaoAtual, minhaDirecao): 
    if(moverY != 0 and notStockLocal(posicaoAtual, moverY, axisY)):
        minhaDirecao = direcaoCorreta(minhaDirecao, moverY, axisY, False)
    elif(moverX != 0 and notStockLocal(posicaoAtual, moverX, axisX)):
        minhaDirecao = direcaoCorreta(minhaDirecao, moverX, axisX, False)
    return minhaDirecao

def irMetadeDoQuadrado(moverX, moverY, posicaoAtual, minhaDirecao):
    if(moverY != 0 and notStockLocal(posicaoAtual, moverY, axisY)):
        minhaDirecao = direcaoCorreta(minhaDirecao, moverY, axisY, True)
        if(moverY > 0): #robô andou para a direita
            posicaoAtual += 1
        else: #robô andou para a esquerda
            posicaoAtual -= 1
    elif(moverX != 0 and notStockLocal(posicaoAtual, moverX, axisX)):
        minhaDirecao = direcaoCorreta(minhaDirecao, moverX, axisX, True)
        if(moverX > 0): #robô andou para baixo
            posicaoAtual += 10
        else:  #robô andou para cima
            posicaoAtual -= 10
    andar_em_metros(adeni, 8, 5, 0.15)
    
    return posicaoAtual, minhaDirecao

def deA_para_B(posicaoAtual, posicaoFinal, minhaDirecao): #Permite que o robô vá de um canto para o outro.
    posicaoFinal = posicaoRealFinal(posicaoFinal)
    i = 0
    while(not chegadaNolocal(posicaoAtual, posicaoFinal)):
        ...
        if(i == 0):
            minhaDirecao = corrigindoDirecao(moverX, moverY, posicaoAtual, minhaDirecao)
            i += 1
        print
        if((abs(moverX) == 1 and abs(moverY) == 0)): #otiimização para andar até a metade do quadrado.
            print('ate metade')
            print(posicaoAtual)
            posicaoAtual, minhaDirecao = irMetadeDoQuadrado(moverX, moverY, posicaoAtual, minhaDirecao)
            print('sai', posicaoAtual)
        elif(moverY != 0 and notStockLocal(posicaoAtual, moverY, axisY)):
            minhaDirecao = direcaoCorreta(minhaDirecao, moverY, axisY, True)
            moverParaFrentePorQuadrado()
            if(moverY > 0): #robô andou para a direita
                posicaoAtual += 1
            ...
            if(moverX > 0): #robô andou para baixo
                ...
                posicaoAtual -= 10
        elif (moverY == 0 and not notStockLocal(posicaoAtual, moverX, axisX)): #o robô ja chegou no eixo Y, mas não pode se movimentar em X por conta da área de carga
            posicaoAtual, minhaDirecao = desvioAreaDeCarga(posicaoAtual, minhaDirecao)
